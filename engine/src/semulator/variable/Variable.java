...
public interface Variable {
    ...
    default boolean isResult() { return getType() == VariableType.RESULT; }
    default boolean isInput()  { return getType() == VariableType.INPUT; }
    default boolean isWork()   { return getType() == VariableType.WORK; }
    ...
    static Variable of(String token) {
        if (token == null) throw new IllegalArgumentException("null variable token");
        String t = token.trim();
        if (t.equals("y")) return RESULT;
        if (t.startsWith("x")) {
